#!/usr/bin/env bash
# make_video.sh — pipeline completo: simulación → video (Film_graphene)
...
#   -o  OUTPUT     nombre del MP4 de salida (default: video_grafeno.mp4)
...
PYTHON_SCRIPT="$ROOT_DIR/python/accumulate_frames.py"
PYTHON_ENV="$HOME/mi_entorno/bin/activate"

N_FRAMES=120
FPS=60
BOX_ALPHA=0.25
OUTPUT="video_grafeno.mp4"
DPI=72
RUN_SIM=1
RUN_CONVERT=1
RUN_REBUILD=1
...
VIDEO_MAC="$ROOT_DIR/macros/vis_video.mac"
BOX_MAC="$ROOT_DIR/macros/vis_box_frame.mac"
OUTPUT_PATH="$BUILD_DIR/$OUTPUT"

sep
echo -e "${BOLD}  make_video.sh — Film Graphene (B-10 Neutron Capture)${RESET}"
sep
log "Frames: $N_FRAMES  |  FPS: $FPS  |  Alpha overlay: $BOX_ALPHA"
log "Salida: $OUTPUT_PATH"
sep

[[ -f "$VIDEO_MAC" ]]    || die "No se encontró: $VIDEO_MAC"
...
[[ -f "$BUILD_DIR/AmBePhaseSpace.root" ]] || \
    warn "AmBePhaseSpace.root no encontrado en build/ — el generador usará pistola de respaldo"
...
if [[ $RUN_REBUILD -eq 1 ]]; then
    ...
    if [[ ! -f "$EXECUTABLE" ]]; then
        warn "Ejecutable no encontrado, compilando..."; NEED_BUILD=1
    else
        if find "$ROOT_DIR/src" "$ROOT_DIR/include" "$ROOT_DIR/main.cc" \
                -newer "$EXECUTABLE" 2>/dev/null | grep -q .; then
            warn "Código fuente modificado, recompilando..."; NEED_BUILD=1
        fi
    fi
...
if [[ $RUN_SIM -eq 1 ]]; then
    ...
    cd "$BUILD_DIR"
    rm -f frame_*.eps box_frame*.eps

    # Frame de referencia de geometría sólida
    if [[ -f "$BOX_MAC" ]]; then
        log "  Generando overlay de geometría..."
        "$EXECUTABLE" "$BOX_MAC" >/dev/null 2>&1
        [[ -f box_frame_0000.eps ]] && ok "  box_frame_0000.eps generado" \
                                    || warn "  box_frame_0000.eps no apareció"
    fi

    "$EXECUTABLE" "$VIDEO_MAC" >/dev/null 2>&1
    N_EPS=$(ls frame_*_0000.eps 2>/dev/null | wc -l)
    ok "Generados $N_EPS archivos EPS"
    [[ $N_EPS -gt 0 ]] || die "No se exportaron frames EPS — revisa el macro"
else
    warn "Simulación omitida (--no-sim)"; cd "$BUILD_DIR"
fi
...
if [[ $RUN_CONVERT -eq 1 ]]; then
    ...
    warn "Conversión omitida (--no-convert)"; cd "$BUILD_DIR"
fi

# ── PASO 3: Acumulación ───────────────────────────────────────────────────────
sep; log "Paso 3 — Acumulación de frames (alpha=$BOX_ALPHA)"
cd "$BUILD_DIR"
source "$PYTHON_ENV"
...
ffmpeg -y -framerate "$FPS" -start_number 1 -i accum_%04d.png \
    -c:v libx264 -pix_fmt yuv420p -crf 18 \
    "$OUTPUT" 2>&1 | grep -E "frame=|kb/s" | tail -2 || true

[[ -f "$OUTPUT" ]] || die "ffmpeg no generó el video"
ok "Video compilado: $OUTPUT (${DURATION}s a ${FPS}fps)"

# ── PASO 5: Limpieza ──────────────────────────────────────────────────────────
sep; log "Paso 5 — Limpieza"
rm -f frame_*.eps frame_*_0000.png frame_[0-9][0-9][0-9][0-9].png \
      accum_*.png box_frame*.eps
